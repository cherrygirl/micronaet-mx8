...
<body>
   ...
   <!-- Start loop for design table for product and material status: -->
     <table class="list_table">      
           <!-- ------------------ HEADER --------------------------------- -->
           <% thead = "<thead><tr><th class='description'>%s</th><th class='description'>%s</th>%s</tr></thead>" %>
           <% thead_internal = "" %>
               %for col in get_cols():
                    <% thead_internal += "<th class='data'>%s</th>"%(col, ) %>                      
               %endfor
           ${thead % ("Materiale", "Iniziale", thead_internal, )}
          <!-- --------------------- BODY --------------------------------- -->
          <!--<tbody>
              <% i=0 %>
              <% rows = get_rows()%>
              %for row in rows:
                  <% status_line = 0.0 %>
                  <tr>
                    <td class="description">${row[0]}</td><!--Description
                    <td class="description">${row[1]}</td><!--Inizial value
                    <% j = 0 %>
                    % for col in get_cols():
